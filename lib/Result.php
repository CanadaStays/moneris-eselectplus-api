...
class Result
{
	 ...
	 * @param Transaction $transaction
	 * @return Result|SecureResult
	 */
	static public function factory(Transaction $transaction)
	{
		$params = $transaction->params();

		if (in_array($params['type'], array('txn', 'acs'))) {
			return new SecureResult($transaction);
		} else {
	 ...
	 *
	 * @param int $code
	 * @return Result|int Fluid interface for set operations.
	 */
	 ...
	 *
	 * @param bool $failed
	 * @return Result|bool Fluid interface on set operations.
	 */
	 ...
	 *
	 * @param Transaction $transaction
	 * @return Result|Transaction Fluid interface for set operations
	 */
	...
	public function validate_response()
	{
		...
		if ('Global Error Receipt' == $receipt->ReceiptId) {
			...
			$this->error_code(Result::ERROR_GLOBAL_ERROR_RECEIPT)
		...
		if ((int) $receipt->ResponseCode >= 50) {
			...
			switch ($receipt->ResponseCode) {
				...
				case '074':
				case 'null':
					$this->error_code(Result::ERROR_SYSTEM_UNAVAILABLE);
					break;
				case '051':
				case '482':
				case '484':
					$this->error_code(Result::ERROR_CARD_EXPIRED);
					break;
				case '075':
					$this->error_code(Result::ERROR_INVALID_CARD);
					break;
				case '076':
				case '079':
				case '080':
				...
				case '082':
				case '083':
					$this->error_code(Result::ERROR_INSUFFICIENT_FUNDS);
					break;
				case '077':
					$this->error_code(Result::ERROR_PREAUTH_FULL);
					break;
				case '078':
					$this->error_code(Result::ERROR_DUPLICATE_TRANSACTION);
					break;
				case '481':
				case '483':
					$this->error_code(Result::ERROR_DECLINED);
					break;
				case '485':
					$this->error_code(Result::ERROR_NOT_AUTHORIZED);
					break;
				case '486':
				case '487':
				case '489':
				case '490':
					...
					$this->error_code(Result::ERROR_CVD);
					break;
				default:
		...
		if ($gateway->check_avs()
			...
			switch ($receipt->AvsResultCode) {
				case 'B':
				case 'C':
					$this->error_code(Result::ERROR_AVS_POSTAL_CODE);
					break;
				case 'G':
				case 'I':
				case 'P':
				...
				case 'U':
				case 'Z':
					$this->error_code(Result::ERROR_AVS_ADDRESS);
					break;
				case 'N':
					$this->error_code(Result::ERROR_AVS_NO_MATCH);
					break;
				case 'R':
					$this->error_code(Result::ERROR_AVS_TIMEOUT);
					break;
				default:
					$this->error_code(Result::ERROR_AVS);
			}
		...
		if ($gateway->check_cvd()
			...
			$this->error_code(Result::ERROR_CVD)->failed_cvd(true);
	 ...
	 * @return bool|Result Fluid interface for set operations
	 */
